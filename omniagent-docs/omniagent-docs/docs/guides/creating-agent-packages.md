...
./tools/agent-pack/build.sh agents/hello-agent
# 输出: target/packages/hello-agent-0.1.0.oap
```
...
fn main() {
    AgentBuilder::new("com.example.file-manager")
        ...
        .handler(handle_message)
        .on_start(|ctx| {
            ctx.log_info("File Manager Agent started");
            ctx.register_port("file-manager").unwrap();
        })
        .build()
        .run();
}
    ...
    match msg.command() {
        ...
        "search" => {
            ...
            match search_files(&query) {
                Ok(results) => Response::ok().data(results),
        ...
        "batch_move" => {
            ...
            let results: Vec<Result<String, String>> = ops.into_iter()
                .map(|op| move_file(&op.source, &op.dest).map_err(|e| e.to_string()))
                .collect();
...
fn main() {
    AgentBuilder::new("com.example.data-analyzer")
        .version("1.0.0")
        .handler(handle_message).build().run();
}
...
fn perform_analysis(req: &AnalysisRequest) -> Result<AnalysisResult, String> { todo!() }
```
...
fn handle_message(ctx: &mut Context, msg: Message) -> Response {
    match msg.command() {
        "read_file" => {
            let path: String = match msg.data() {
                Ok(p) => p, Err(_) => return Response::error("invalid path"),
            };
...
```rust
fn handle_message(ctx: &mut Context, msg: Message) -> Response {
    let request: MyRequest = match msg.data() {
        Ok(r) => r,
        Err(e) => { ctx.log_error(&format!("Invalid: {}", e)); return Response::error("invalid"); }
    };
    match ctx.request_with_timeout("fsd", &msg, 5000) {
        Ok(response) => Response::ok().data(response),
        Err(TimeoutError) => { ctx.log_warn("FSD timeout"); Response::error("unavailable") }
        Err(e) => { ctx.log_error(&format!("Error: {}", e)); Response::error("internal error") }
    }
}
```
