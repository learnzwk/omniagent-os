            ...
            match omniagent_ipc::select(&[&self.port, &self.irq_port]) {
                Ok(event) => match event.source() {
                    &self.port => {
                        ...
                        let response = driver.handle_message(&msg)?;
                        reply.send(&response)?;
                    }
                    &self.irq_port => {
...
pub fn register_driver(driver: &mut dyn DeviceDriver) -> Result<(), DriverError> {
    ...
    for (start, end) in &caps.io_ports {
        omniagent_syscall::request_io_port(*start, *end - *start)?;
    }
    for (phys_addr, size) in &caps.mmio_regions {
        omniagent_syscall::map_device_memory(*phys_addr, *size)?;
    }
...
// 中断处理最佳实践
fn handle_interrupt(&mut self, irq: u8) -> Result<(), DriverError> {
    self.acknowledge_interrupt(irq)?;  // 1. ACK 中断
    let status = self.read_status()?;  // 2. 读取状态
    if status & STATUS_RX_READY != 0 { self.handle_rx_ready()?; }
    if status & STATUS_TX_COMPLETE != 0 { self.handle_tx_complete()?; }
    if status & STATUS_ERROR != 0 { self.handle_error(status)?; }
    self.enable_interrupts(irq)?;      // 3. 重新启用
    Ok(())
}
```
...
// 集成测试使用模拟设备
struct MockBus { present: bool }
#[test]
fn test_probe_with_mock() {
    assert!(SerialDriver::new().probe(&MockBus { present: true }).is_ok());
}
```
...
1. 分析 Linux 驱动的初始化流程和数据结构
2. 创建 Agent 项目，实现 `DeviceDriver` trait
3. 替换内核 API：`ioread32` -> `IoPort::read_u32`，`request_irq` -> `register_irq`
4. 适配中断模型：从硬中断改为 IPC 消息处理
5. 编写模拟设备测试
6. 性能优化
