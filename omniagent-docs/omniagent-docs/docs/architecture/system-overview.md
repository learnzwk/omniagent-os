...
│ (推理任务)   │
└──────┬───────┘
       │ Agent Syscall 513 (ai_inference)
       ▼
┌──────────────────────────────────────────┐
│           AI 推理服务 (Router)            │
│  ┌────────────────────────────────────┐  │
...
pub mod agent_syscall {
    pub const AGENT_CREATE:        usize = 512; // 创建新 Agent
    pub const AGENT_DESTROY:       usize = 513; // 销毁 Agent
    pub const AGENT_SPAWN:         usize = 514; // 启动 Agent 执行
    pub const AGENT_SUSPEND:       usize = 515; // 挂起 Agent
    pub const AGENT_RESUME:        usize = 516; // 恢复 Agent
    pub const AGENT_SEND:          usize = 517; // Agent 间消息发送
    pub const AGENT_RECV:          usize = 518; // Agent 间消息接收
    pub const AGENT_SHARE_MEM:     usize = 519; // 创建共享内存区域
    pub const AGENT_SET_QUOTA:     usize = 520; // 设置资源配额
    pub const AGENT_GET_STATUS:    usize = 521; // 查询 Agent 状态
    pub const AGENT_REGISTER:      usize = 522; // 注册 Agent 能力
    pub const AGENT_DISCOVER:      usize = 523; // 发现可用 Agent
    pub const AGENT_ENCLAVE_ENTER: usize = 524; // 进入安全 Enclave
    pub const AGENT_ENCLAVE_EXIT:  usize = 525; // 退出安全 Enclave
    pub const AI_INFERENCE:        usize = 526; // AI 推理请求
    pub const AI_MODEL_LOAD:       usize = 527; // 加载 AI 模型
    pub const AI_MODEL_UNLOAD:     usize = 528; // 卸载 AI 模型
}
```
...
| Agent 创建时间 | < 100μs | 从 `AGENT_CREATE` 到 `RUNNING` |
